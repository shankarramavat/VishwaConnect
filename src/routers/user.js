...
const jwt = require("jsonwebtoken");//session store
...
router.get("/questions", async (req, res) => { //async javascript to handle clash 
  let category = req.query.cat.toString();
  if (category.includes("%20")) {
    category = category.replace("%20", " ");
  }
  const questions = await Question.find({ category: req.query.cat });//user enter catego
...
router.get("/questions/:userid", async (req, res) => { //individual question answer
  let category = req.query.cat.toString();
  if (category.includes("%20")) {
    category = category.replace("%20", " ");
  }
...
router.get("/answers/:id", async (req, res) => { //multiple answeer to a particular question
...
router.post("/register", async (req, res) => {
  ...
  if (emailExist) {
    ...
    const user = new User({
      ...
      password: hashPassword,//log password
    });
    try {
      const savedUser = await user.save();
      console.log(savedUser);
      const token = jwt.sign({ _id: savedUser._id }, process.env.JWT_SECRET);//password transaction route protection
